...
(() => {
  ...
  // desktop-v2/personalities/preview/player-entry.tsx
...
`;
  ...
  // src/remotion/personalities/PersonalitiesComposition.tsx
  ...
  var isVideoUrl = (value) => /\.(mp4|mov|webm|m4v|mkv)$/i.test(value || "");
  var getTrimmedDurationMs = (scene, fallbackSceneDurationMs) => {
    ...
    if (scene.mediaType !== "video") {
      return Math.max(1e3, Number(scene.voiceoverDurationMs || 0) || fallbackSceneDurationMs);
    }
    if (trimEndMs != null && trimEndMs > trimStartMs) {
      return Math.max(1e3, trimEndMs - trimStartMs);
    }
    if (mediaDurationMs > trimStartMs) {
      return Math.max(1e3, mediaDurationMs - trimStartMs);
    }
    return Math.max(1e3, fallbackSceneDurationMs);
  };
  var getSceneDurationMs = (scene, fallbackSceneDurationMs) => {
    ...
    if (scene.mediaType === "video") {
      if (voiceoverDurationMs > 0) {
        return Math.max(1e3, Math.min(trimmedDurationMs, voiceoverDurationMs));
      }
      return trimmedDurationMs;
    }
    if (voiceoverDurationMs > 0) {
      return Math.max(1e3, voiceoverDurationMs);
    }
    return Math.max(1e3, fallbackSceneDurationMs);
  };
  var SceneMedia = ({ scene, placeholderUrl, durationInFrames }) => {
    const sourceUrl = scene.fileUrl || placeholderUrl;
    if (!sourceUrl) {
      return /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { backgroundColor: "#05070b" } });
    }
    if (scene.mediaType === "video" || isVideoUrl(sourceUrl)) {
      const startFrom = Math.max(0, Math.round(Number(scene.trimStartMs || 0) / 1e3 * FPS));
      const endFromTrim = scene.trimEndMs == null ? null : Math.max(startFrom + 1, Math.round(Number(scene.trimEndMs || 0) / 1e3 * FPS));
  ...
  var SceneLayer = ({ scene, index, placeholderUrl }) => {
    const frame = useCurrentFrame();
    ...
    return /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { overflow: "hidden", opacity } }, /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { transform: `scale(${scale})`, transformOrigin: "center center" } }, /* @__PURE__ */ import_react4.default.createElement(SceneMedia, { scene, placeholderUrl, durationInFrames })), /* @__PURE__ */ import_react4.default.createElement(
      AbsoluteFill,
      {
        style: {
  ...
  var EndPageFade = ({ src, fadeFrames }) => {
    ...
    if (isVideoUrl(src)) {
      return /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { backgroundColor: "#000", opacity } }, /* @__PURE__ */ import_react4.default.createElement(AdaptiveVideo, { src, muted: true, style: { width: "100%", height: "100%", objectFit: "cover" } }));
    }
    return /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { backgroundColor: "#000", opacity } }, /* @__PURE__ */ import_react4.default.createElement(Img, { src, style: { width: "100%", height: "100%", objectFit: "cover" } }));
  };
  var PersonalitiesComposition = ({
    scenes,
    placeholderUrl = null,
    musicUrl = null,
    musicVolume = 0.3,
    ...
    const sceneEndFrame = sequenceData.length ? Math.max(...sequenceData.map((item) => item.from + item.durationInFrames)) : 0;
    const endPageFadeFrames = Math.min(Math.round(FPS * 1.5), Math.max(8, endPageDurationFrames));
    const endPageStartFrame = endPageUrl && endPageDurationFrames > 0 ? Math.max(0, sceneEndFrame - endPageFadeFrames) : sceneEndFrame;
    const totalFrames = endPageUrl && endPageDurationFrames > 0 ? sceneEndFrame + endPageDurationFrames - endPageFadeFrames : sceneEndFrame;
    return /* @__PURE__ */ import_react4.default.createElement(AbsoluteFill, { style: { backgroundColor: "#000", direction: "ltr" } }, sequenceData.map((item, index) => /* @__PURE__ */ import_react4.default.createElement(
      Sequence,
      {
      ...
      item.scene.voiceoverUrl ? /* @__PURE__ */ import_react4.default.createElement(
        Audio,
        {
          src: item.scene.voiceoverUrl,
          volume: typeof voiceoverVolume === "number" && !Number.isNaN(voiceoverVolume) ? voiceoverVolume : 1
        }
      ) : null
    )), endPageUrl && endPageDurationFrames > 0 ? /* @__PURE__ */ import_react4.default.createElement(Sequence, { from: endPageStartFrame, durationInFrames: endPageDurationFrames, layout: "none" }, /* @__PURE__ */ import_react4.default.createElement(EndPageFade, { src: endPageUrl, fadeFrames: endPageFadeFrames })) : null, musicUrl ? /* @__PURE__ */ import_react4.default.createElement(
      Audio,
      {
        src: musicUrl,
        loop: true,
    ...
    ) : null, /* @__PURE__ */ import_react4.default.createElement(VisualEffects, { effects, cinematicBarSize }));
  };

  // desktop-v2/personalities/preview/player-entry.tsx
  var FPS2 = 25;
  var PLAYER_STYLE = { width: "100%" };
