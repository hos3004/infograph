...
async function run() {
  ...
  const entryPoint = path.join(repoRoot, 'src', 'remotion', 'personalities', 'index.ts');
  const stagedPublicDir = await prepareRemotionPublicDir(desktopPaths);
  const outDir = process.env.DESKTOP_V2_BUNDLE_OUT_DIR_PERSONALITIES || path.join(buildRoot, 'bundle-staging-personalities', 'remotion-bundle');
  ...
  console.log('Desktop V2 Personalities Remotion bundle ready at:', serveUrl);
}

run().catch((error) => {
  console.error('Desktop V2 Personalities bundle build failed:', error);
  process.exit(1);
});
